	...
	Object.keys(browsers).map(function(type) {
		...
		if (typeof browsers[type].variants === 'string') {
			var check = useDarwinCheck(type) ?
				...
				checkOthers.bind(undefined, browsers[type], regex);
			pushCheck(check, type, type, browsers[type]);
			return;
		}

		Object.keys(multiBrowser).forEach(function(name) {
			...
			if ( !err ) {
				if ( Array.isArray( version ) ) {
					...
					version.forEach( function( item ) {
						available.push({
							name: flatBrowser.name,
							type: flatBrowser.type,
					...
					available.push({
						...
						type: flatBrowser.type,
						version: version,
						command: path || flatBrowser.command
					});
				}
			}
